    ...
    <script runat="server">
        ...
        class GoogleSearchBox
        {
            ...
            public String GetIdentity()
            {
                ...
                log("Current User Name before conversion = " + username, LOG_LEVEL.INFO);
                SPClaimProviderManager mgr = SPClaimProviderManager.Local;
                if (mgr != null && SPClaimProviderManager.IsEncodedClaim(username))
                {
                    username = mgr.DecodeClaim(username).Value;
                }
                log("User Name after conversion = " + username, LOG_LEVEL.INFO);
                return username;        
            }
            ...
            <td id="TD1" colspan="4" >
                ...
                <%
                    ...
                    { 
                         ...
                         */
                        String filterParameter = !String.IsNullOrEmpty(WebConfigurationManager.AppSettings["filterParameter"]) ? WebConfigurationManager.AppSettings["filterParameter"] : String.Empty;
                        searchReq = "?q=" + HttpUtility.UrlEncode(qQuery) + "&access=" + gProps.accessLevel + "&getfields=*&output=xml_no_dtd&ud=1" 
                            + "&oe=UTF-8&ie=UTF-8&exclude_apps=1&site=" + gProps.siteCollection
                    ...
                    {
                        // When incoming request is generated by user clicks on Next or previous 
                        // links on search results page search query is already formed and 
                        // available as query string
                        ...
                        //TODO : Cleanup code and avoid duplicate code.
                        if (!String.IsNullOrEmpty(inquery["access"]))
                        {
                            string access = inquery["access"].ToString();
                            if (access == "a")
                            {
                                gProps.accessLevel = "a"; // Perform 'public and secure search'
                            }
