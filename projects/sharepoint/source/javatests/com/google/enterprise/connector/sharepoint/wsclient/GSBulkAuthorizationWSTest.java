...
public class GSBulkAuthorizationWSTest extends TestCase{
	...
	public void testBulkAuthorize() throws Throwable{
		...
		StringTokenizer strTok = new StringTokenizer(TestConfiguration.SearchDocID1,SPConstants.DOC_TOKEN);
		if(strTok!=null){
			...
			inputDocs[0] = new AuthData(listURL,DocID,false,"", TestConfiguration.SearchDocID1);
		}
		strTok = new StringTokenizer(TestConfiguration.SearchDocID2,SPConstants.DOC_TOKEN);
		if(strTok!=null){
			//String docURL = strTok.nextToken();
			final String listURL = strTok.nextToken();
			final String DocID = strTok.nextToken();
			//inputDocs[1] = new AuthData(docURL,listID,DocID,false,"");
			inputDocs[1] = new AuthData(listURL,DocID,false,"", TestConfiguration.SearchDocID2);
		}
		strTok = new StringTokenizer(TestConfiguration.SearchDocID3,SPConstants.DOC_TOKEN);
		if(strTok!=null){
			//String docURL = strTok.nextToken();
			final String listURL = strTok.nextToken();
			final String DocID = strTok.nextToken();
			//inputDocs[2] = new AuthData(docURL,listID,DocID,false,"");
			inputDocs[2] = new AuthData(listURL,DocID,false,"", TestConfiguration.SearchDocID3);
		}
