 ...
 */
class SharePointDAO extends SimpleJdbcDaoSupport {
  private static final Logger LOGGER =
      Logger.getLogger(SharePointDAO.class.getName());
  ...
  protected SharePointDAO(DataSource dataSource,
   ...
   * @throws SharepointException
   */
  private int[] handleBatchUpdateExceptionForOracle(
   ...
   */
  @VisibleForTesting
  int[] handleBatchUpdateExceptionForMSSQLAndMySQL(
          ...
          for (int i = 0; i < optimisticBatchStatus.length; i++) {
            try {
              ...
              LOGGER.log(Level.WARNING, "Failed to add record to user data store with the group name ["
                  + params[i].getValue(GROUP_NAME)
                  + ", user name ["
                  + params[i].getValue(USER_NAME)
                  + "]", e);
            }
          }
        }
