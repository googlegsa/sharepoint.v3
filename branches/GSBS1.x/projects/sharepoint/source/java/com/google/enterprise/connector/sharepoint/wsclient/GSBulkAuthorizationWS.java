...
/**
 * Java Client for calling GSBulkAuthorization.asmx.
 * Provides a layer to talk to the GSBulkAuthorization Web Service deployed on the SharePoint server. 
...
public class GSBulkAuthorizationWS {
	 ...
	 */
	public GSBulkAuthorizationWS(final SharepointClientContext inSharepointClientContext)
	throws SharepointException {
		
		if(inSharepointClientContext!=null){
			sharepointClientContext=inSharepointClientContext;
			endpoint = Util.encodeURL(sharepointClientContext.getSiteURL()) + SPConstants.GSPBULKAUTHORIZATION_ENDPOINT;
	 ...
	 * @return the updated AuthData object reflecting the authorization status for each document
	 * @throws RemoteException
	 */
	public AuthData[] bulkAuthorize(final AuthData[] authData, final String loginId) throws RemoteException{
		...
		try {
			...
			if(SPConstants.UNAUTHORIZED.indexOf(af.getFaultString()) != -1) {
				...
				try {
					...
					LOGGER.log(Level.WARNING, "Problem while making remote call to BulkAuthorize. endpoint [ "+endpoint+" ]",e);											
				}
			} else {
				LOGGER.log(Level.WARNING, "Problem while making remote call to BulkAuthorize. endpoint [ "+endpoint+" ]",af);					
			}
	 ...
	 * @return The connectivity status "success" if succeed or the reason for failure.
	 */
