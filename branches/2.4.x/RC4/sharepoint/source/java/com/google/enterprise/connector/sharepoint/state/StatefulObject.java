 ...
 * canonical example of a StatefulObject; a List corresponds to a SPDocument
 * Library and other SharePoint containers. A StatefulObject must have at least
 * two attributes: 1) a PrimaryKey, which is unique across all instances of that
 * object. 2) a LastMod date, which represents the time the underlying
 * SharePoint was last modified. LastMod need not be unique. A StatefulObject
 * must dump its state to a DOM object when asked, and restore its state from a
 * DOM object. A master object (GlobalState) is responsible for assembling those
 * DOM objects into a master DOM, which it dumps to XML and restores from XML.
 */
