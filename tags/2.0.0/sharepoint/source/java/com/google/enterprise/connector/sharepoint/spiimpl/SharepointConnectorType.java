 ...
 * This class is mainly desinged for controlling the connector configuration which incompasses
 * 	creation of connector configuration form, validating the configuration values etc.
...
public class SharepointConnectorType implements ConnectorType {
	...
	private void setConfigStrings() {
		if(rb!=null){
			...
			LOGGER.config("Config Strings loaded from the resource bundle : "+configStrings.toString());
		}else{
		...
		if(keys!=null){
			for (final Iterator itr = keys.iterator(); itr.hasNext();) {
							...
							for(final Iterator aliasItr=aliasValues.iterator();aliasItr.hasNext();){
								...
								if(aliases.size()==0) {
									...
									try {										
										...
										final String logMessage = "Could not find the alias value for the pattern ["+alias_source_pattern+"].";
				...
				if(collator.equals(key, SPConstants.AUTHORIZATION)) {
					...
					if((value.length() == 0) || value.equalsIgnoreCase(SPConstants.METADATA_URL_FEED)) {
				...
				} else if((collator.equals(key,SPConstants.EXCLUDED_URLS)) || (collator.equals(key,SPConstants.INCLUDED_URLS))){
					buf.append(SPConstants.OPEN_ELEMENT);
					...
					if(collator.equals(key,SPConstants.SHAREPOINT_URL) /*|| collator.equals(key,DOMAIN)*/ || collator.equals(key,SPConstants.MYSITE_BASE_URL)) {
						appendAttribute(buf, SPConstants.TEXTBOX_SIZE, SPConstants.TEXTBOX_SIZE_VALUE);
					}
					buf.append(SPConstants.SLASH + SPConstants.CLOSE_ELEMENT);
				}
	...
	private void appendTableForAliasMapping(final StringBuffer buf) {
		...
		buf.append(SPConstants.OPEN_ELEMENT + SPConstants.TH + SPConstants.CLOSE_ELEMENT);
		buf.append(SPConstants.TH_END);
	 ...
	 */
	private void appendRowForAliasMapping(final StringBuffer buf, final String sourceValue, final String aliasValue, final boolean color) {
		buf.append(SPConstants.TR_START);
	...
	/**
	 * Starts a new row in the configuration page. used while designing the configuration form
	 * @param buf
	 */
	private void appendStartRow(final StringBuffer buf, final String key, final String configKey,final boolean red/*, final String displayValue*/) {
	...
	/**
	 * Adds a property to the current HTML element. used while designing the configuration form
	 * @param buf
	 * @param attrName
	 * @param attrValue
	 */
	private void appendAttribute(final StringBuffer buf, final String attrName,	final String attrValue) {
		buf.append(" ");
		buf.append(attrName);
		buf.append("=\"");
	...
	private boolean validateConfigMap(final Map configData, final ErrorDignostics ed) {
		...
		for (final Iterator i = keys.iterator(); i.hasNext();) {
			...
			if (isRequired(key)){
				if((val==null) || val.equals(SPConstants.BLANK_STRING) || (val.length()==0)) {
					ed.set(key, rb.getString(SPConstants.REQ_FIELDS_MISSING)+SPConstants.SPACE+rb.getString(key));
					return false;
				} else if(collator.equals(key,SPConstants.SHAREPOINT_URL)) {
					if(!isURL(val)) {
				...
				final Set invalidSet = validatePatterns(val);
				if(invalidSet != null) {
					ed.set(SPConstants.EXCLUDED_URLS, rb.getString(SPConstants.INVALID_EXCLUDE_PATTERN) + invalidSet.toString());
					return false;
				}
		...
		if((username != null) && ((username.indexOf("@") != -1) || (username.indexOf("\\") != -1)) && (domain != null) && !domain.equals(SPConstants.BLANK_STRING)) {
		...
		if(SPConstants.CONTENT_FEED.equalsIgnoreCase(feedType)) {
			...
			if(!SPConstants.CONNECTIVITY_SUCCESS.equalsIgnoreCase(status)) {
				ed.set(null, rb.getString(SPConstants.BULKAUTH_ERROR_CRAWL_URL) + rb.getString(SPConstants.REASON) + status);
				return false;			
			}
		} 
		status = null;
		...
		if(SPConstants.SP2007.equals(SPVersion) && (mySiteUrl!=null) && !mySiteUrl.equals(SPConstants.BLANK_STRING)) {
			if(!isURL(mySiteUrl)) {
			...
			if(!SPConstants.CONNECTIVITY_SUCCESS.equalsIgnoreCase(status)) {
				ed.set(SPConstants.MYSITE_BASE_URL, rb.getString(SPConstants.CANNOT_CONNECT_MYSITE) + rb.getString(SPConstants.REASON) + status);
				return false;			
			}
			...
			if(SPConstants.CONTENT_FEED.equalsIgnoreCase(feedType)) {
				...
				if(!SPConstants.CONNECTIVITY_SUCCESS.equalsIgnoreCase(status)) {
					ed.set(SPConstants.MYSITE_BASE_URL, rb.getString(SPConstants.BULKAUTH_ERROR_MYSITE_URL) + rb.getString(SPConstants.REASON) + status);
					return false;			
				}			
			}
		}				
	...
	/**
	 * Used while re-displaying the connector configuration page after any validation error occurs. 
	 */	
	private ConfigureResponse makeValidatedForm(final Map configMap, final ErrorDignostics ed) 
	{
		final String sFunName=className+".makeValidatedForm(final Map configMap, ErrorDignostics ed)";
		if(configMap==null){
	...
	public ConfigureResponse getConfigForm(final Locale locale) {
		...
		setConfigStrings();
		final ConfigureResponse result = new ConfigureResponse("", getInitialConfigForm());

		return result;
	}

	/**
	 * Called by connector-manager to display the connector configuration page with filled in values.
	 */
	public ConfigureResponse getPopulatedConfigForm(final Map configMap, final Locale locale)  {
		...
		rb = ResourceBundle.getBundle("SharepointConnectorResources", locale);
		setConfigStrings();
		final ConfigureResponse result = new ConfigureResponse("", makeConfigForm(configMap,null));
	 ...
	 */
//	due to GCM changes
	public ConfigureResponse validateConfig(final Map configData, final Locale locale, final ConnectorFactory arg2) {
	...
	/**
	 * Stores any error message corresponding to any field on the connector's configuration page.	 *  
	 * @author nitendra_thakur
	 */
	...
	private boolean isRequired(final String configKey){
		if(collator.equals(configKey,SPConstants.SHAREPOINT_URL) || collator.equals(configKey,SPConstants.USERNAME) || 
				collator.equals(configKey,SPConstants.PASSWORD) || collator.equals(configKey,SPConstants.INCLUDED_URLS)){
			return true;
		} else {
			return false;
		}
	...
	*/
	...
	private boolean isURL(final String url) {
		...
		try {
			...
			/*
			 * Check for port value greater then allowed range is added beacuse of a bug in Axis where a NullPointer Exception 
			 * was being returned as a vlaue during Web Service call.
			 */
	...
	/**
	 * Matches the provided url against included and excluded patterns
	 * Function Signature changed by nitendra_thakur. This is to make the function re-usable. 
	 */
	private String checkPattern(final String url){
		...
		if(Util.match(includeList, url, null)) {
			...
			if((excludeList != null) && Util.match(excludeList, url, matchedPattern)) {
				return rb.getString(SPConstants.EXCLUDED_PATTERN_MATCH) + matchedPattern.toString();
			}
			return null;
		} else {
			return rb.getString(SPConstants.INCLUDED_PATTERN_MISMATCH);
	...
	/**
	 * Validates all the patterns under included / excluded to check if any of them is not a valid pattern.
	...
	private Set validatePatterns(final String patterns) {
		...
		for (final String pattern : patternsList) {
			...
			if(pattern.startsWith(SPConstants.REGEXP_IGNORE_CASE)){
				...
				try {
					new RE(strRegexIgnoreCasePattrn,RE.REG_ICASE); // ignore case
				} catch (final Exception e) {
					invalidPatterns.add(pattern);
				}
				continue;
			...
			if(containProtocol){
				...
				if(patternDecoded.indexOf(SPConstants.SLASH)!= -1){
					if(patternDecoded.indexOf(SPConstants.COLON) == -1){
						urlPatt1stPart = patternDecoded.substring(0,patternDecoded.indexOf(SPConstants.SLASH))+":[0-9].*";
					}else{
	...
	private String checkGSConnectivity(final String endpoint) {
		...
		try {
			sharepointClientContext.setSiteURL(endpoint);
			final GSBulkAuthorizationWS testBulkAuth = new GSBulkAuthorizationWS(sharepointClientContext);
	...
	*/
	private String parseAlias(final String aliasMapString, Set<String> wrongEntries) {	
		LOGGER.config("parsing aliasString: "+aliasMapString);
		aliasMap = null;
		if((aliasMapString==null) || aliasMapString.equals(SPConstants.BLANK_STRING)) {
			return null;
		}		
		String message = null;
		...
		try {
				...
				if((nextEntry==null) || nextEntry.equals(SPConstants.BLANK_STRING)) {
					...
					try {
						...
						if(alias_entry.length!=2) {
							LOGGER.warning("Skipping alias entry [ "+nextEntry+" ] because required values are not found.");
							continue;
						} else {						
							...
							if((null == message) || message.equals(SPConstants.MALFORMED_ALIAS_PATTERN)) {
								if(!isURL(sourceURL)) {
							...
							if(aliasMap.containsKey(source_url)) {
								...
								if((null == message) || message.equals(SPConstants.DUPLICATE_ALIAS)) {
						...
						final String logMessage = "Exception thrown while parsing AliasMap [ "+aliasMapString+" ] Values:";
						LOGGER.log(Level.WARNING, logMessage, e);					
						continue;
					}
				}			
			...
			LOGGER.log(Level.WARNING,"Problem while parsing alias string [ "+aliasMapString+" ].",e);
		}
		return message;
	}	
}
